...
<div class="min-h-screen bg-[#0a0a0f] text-white" style="font-family:'DM Sans',sans-serif;">
    ...
    <div class="max-w-6xl mx-auto px-6 py-8">
        ...
        <div class="grid grid-cols-3 lg:grid-cols-8 gap-4">
            @foreach($finishedBooks as $readingLog)
            @php
        ...
        {{-- Pagination --}}
        @if($finishedBooks->hasPages())
        <div class="mt-8 pt-8 border-t border-white/5">
            <div class="flex items-center justify-between">
                <div class="text-sm text-slate-500">
                    Showing {{ $finishedBooks->firstItem() }} to {{ $finishedBooks->lastItem() }} of {{ $finishedBooks->total() }} books
                </div>
                <div class="flex gap-2">
                    {{-- Previous Page --}}
                    @if ($finishedBooks->onFirstPage())
                        <span class="px-4 py-2 bg-slate-800 text-slate-600 rounded-sm cursor-not-allowed">
                            Previous
                        </span>
                    @else
                        <a href="{{ $finishedBooks->previousPageUrl() }}" 
                    ...
                    {{-- Page Numbers --}}
                    @foreach(range(1, $finishedBooks->lastPage()) as $page)
                        @if($page == $finishedBooks->currentPage())
                            <span class="px-4 py-2 bg-purple-600 text-white rounded-sm">
                                {{ $page }}
                            </span>
                        @else
                            <a href="{{ $finishedBooks->url($page) }}" 
                               class="px-4 py-2 bg-slate-800 text-white rounded-sm hover:bg-purple-600 transition">
                                {{ $page }}
                            </a>
                        @endif
                    ...
                    {{-- Next Page --}}
                    @if ($finishedBooks->hasMorePages())
                        <a href="{{ $finishedBooks->nextPageUrl() }}" 
                           class="px-4 py-2 bg-slate-800 text-white rounded-sm hover:bg-purple-600 transition">
                            Next
                        </a>
                    @else
