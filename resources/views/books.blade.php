...
<div class="bg-slate-900 min-h-screen pt-24 pb-16 px-6 lg:px-32">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            @if(request('search'))
                ...
                <h2 class="text-2xl font-black text-white italic">
                    ...
                    <span class="text-slate-500 font-normal text-base not-italic">
                        — {{ count($popularBooks) }} buku ditemukan
                    </span>
                </h2>
            @else
                <p class="text-slate-400 text-sm mb-1 uppercase tracking-widest font-medium">Sedang trending</p>
                <h2 class="text-2xl font-black text-white italic">Popular Books</h2>
            @endif
        ...
        <form action="/books" method="GET" id="filter-form" class="flex gap-3 flex-wrap">
            ...
            <select name="year" onchange="document.getElementById('filter-form').submit()"
                    class="bg-slate-800 text-slate-300 border border-slate-700 rounded-lg px-4 py-2 text-sm outline-none
            ...
            <select name="genre" onchange="document.getElementById('filter-form').submit()"
                ...
                @foreach([
                    ...
                    <option value="{{ $val }}" {{ request('genre') == $val ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        </form>
    </div>
    ...
    @if(count($popularBooks) > 0)
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5 mb-20">
            @foreach($popularBooks as $book)
                @php
                    $externalId = str_replace('/works/', '', $book['key']);
                @endphp
                <a href="{{ route('books.show', $externalId) }}" class="group flex flex-col">
                    {{-- Cover --}}
                    <div class="aspect-[3/4] bg-slate-800 rounded-lg overflow-hidden mb-3
                                border border-slate-700 transition-all duration-300
                                group-hover:border-purple-500 group-hover:shadow-lg group-hover:shadow-purple-900/30
                    ...
                    @if(isset($book['first_publish_year']))
                        <p class="text-slate-600 text-[10px] mt-0.5">{{ $book['first_publish_year'] }}</p>
                    @endif
                </a>
            @endforeach
        </div>
    @else
        ...
        <div class="flex flex-col items-center justify-center py-24 text-center">
            ...
            <a href="/books" class="px-5 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm transition">
                Reset Pencarian
            </a>
        </div>
    @endif
