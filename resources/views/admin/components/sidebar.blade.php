<aside class="w-64 bg-slate-900 border-r border-white/10 overflow-y-auto transition-all duration-300" :class="!sidebarOpen && '-ml-64'">
        ...
        <nav class="space-y-2">
            ...
            <div x-data="{ open: {{ request()->routeIs('admin.books.*') ? 'true' : 'false' }} }">
                ...
                <div x-show="open" class="mt-2 ml-2 space-y-1 pl-2 border-l border-slate-700">
                    <a href="{{ route('admin.books.index') }}" class="block px-4 py-2 rounded-lg {{ request()->routeIs('admin.books.index') ? 'bg-purple-600/20 text-purple-400' : 'text-slate-400 hover:text-white hover:bg-slate-800' }} text-sm transition-colors">
                        All Books
                    </a>
                    <a href="{{ route('admin.books.create') }}" class="block px-4 py-2 rounded-lg {{ request()->routeIs('admin.books.create') ? 'bg-purple-600/20 text-purple-400' : 'text-slate-400 hover:text-white hover:bg-slate-800' }} text-sm transition-colors">
                        Add Book
                    </a>
                </div>
            </div>
            ...
            <!-- Reports -->
            <a href="{{ route('admin.reports') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('admin.reports') ? 'bg-purple-600/20 text-purple-400 border border-purple-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800' }} transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Settings -->
            <a href="{{ route('admin.settings') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('admin.settings') ? 'bg-purple-600/20 text-purple-400 border border-purple-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800' }} transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-6 border-t border-slate-700">
        <div class="space-y-2">
            <a href="{{ route('home') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
